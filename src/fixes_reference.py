...
def _safe_granger(frame: pd.DataFrame, maxlag: int) -> float:
    ...
    try:
        with warnings.catch_warnings(), contextlib.redirect_stdout(io.StringIO()):
            ...
            res = grangercausalitytests(frame, maxlag=maxlag)  # verbose kwarg deprecated; suppress stdout
...
def price_discovery_daily_by_type(
    ...
    for (name, otype), g in panel.groupby(["underlying_name", "opt_type"]):
        ...
        opt_by_date = band.groupby("date_dt").apply(_vw, include_groups=False).rename("option_ret")
        und_by_date = (
...
def lob_file_option_share(lob_daily: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    for name, g in lob_daily.groupby("underlying_name", dropna=True):  # dropna=True: P7
